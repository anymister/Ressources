...
public class RepertoireEvenement {
	...
	public void enregistrerEvenement(Evenement event) {
		try {
			...
			for (int i = 0; i < event.getRepContact().getNomEmployer().size(); i++) { 
				contact.write(event.getRepContact().nomEmploye().get(i) + ",");
...
public Evenement lireEvenement(int heure) {
	String line = "";
	try {
		repertoireEvenement.clear();
	...
	Evenement event = new Evenement(null, null, null, null);
